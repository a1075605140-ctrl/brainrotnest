...
export default function TungTungRhythm() {
  ...
  const speedLabel = `TUNG TUNG × ${Math.round((bpm - BPM_START) / BPM_STEP) + 1}`
          ...
          {PAD_LABELS.map((_, i) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <span className="absolute inset-0 rounded-2xl animate-ping bg-white/20 pointer-events-none" />}
              </button>
            )
          })}
        </div>
